...
int main ( int argc, char *argv[] ) {
    ...
    /*
     * if (((double) field.number)/((double) imax) != field.number/imax)
     * imax=field.number; 
     */
    ...
    fprintf ( fr,
              "%%%%Creator: LightPipes (C) 1993 Gleb Vdovin, beta version 1995 \n" );
    /*
     * fprintf(fr,"%%%%Pages: 1\n");
     */
    ...
    for ( i = 1; i <= field.number - istep; i += istep ) {
        for ( j = 1; j <= field.number - istep; j += istep ) {
            ...
            sum = sum / ( istep * istep );
            i0 = ( int )
                floor ( pow ( ( sum / max_int ), 1. / ( gamma + 0.00001 ) )
                        * 255 );
            /*
...
contrast in low intensities, default G=2.0\n\n",
              arr );
    fprintf ( stderr,
              "Output file F can be printed with any postscript device\n\n" );
