...
void error_print(char *);
  ...
  if((strstr(argv[2], "sam"))!= NULL ) {}
    ...
    if(fwrite ((char *)&(field.fft_level), sizeof(int), 1, stdout) != 1){
        fprintf(stderr,"interpol: Error while writing FIELD.fft_level\n");
        exit(1);
    }

    if(fwrite ((char *)&(field.sph_coords_factor), sizeof(double), 1, stdout) != 1){
        fprintf(stderr,"interpol: Error while writing FIELD.sph_coords_factor\n");
        exit(1);
    }
...
double int16( double xc,  double yc,  double xp,  double yp, double dx,
...
double int4(double x2, double dx, double y1, double y2, double y3, double y4, double xz)
{
double  t0,a,b,c,d;
