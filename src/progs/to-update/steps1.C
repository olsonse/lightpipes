    ...
    for ( istep = 1; istep <= nstep; istep++ ) {
        ...
        for ( jj = 2; jj <= field.number - 2; jj += 2 ) {
            ...
            for ( i = 2; i <= field.number - 1; i++ ) {
                ...
                uij1 =
                    Complex ( field.real[ikij1], field.imaginary[ikij1] );
                uij_1 =
                    Complex ( field.real[ikij_1],
                              field.imaginary[ikij_1] );
            ...
            for ( i = 1; i <= field.number; i++ ) {
                ...
                /*
                 * medium.r =
                 * -4*Pi*Pi*(refr[ik]*refr[ik]-1.)/field.lambda/field.lambda; 
                 */ medium.r = 0.;
                ...
                if ( i <= i_left ) {
                    int iii = i_left - i;
                    c[i].i +=
                        ( AA * 2. * Pi / field.lambda ) *
                        pow ( ( double ) iii / ( ( double ) ( i_left ) ),
                              band_pow );
                }
                if ( i >= i_right ) {
                    int iii = i - i_right;
        ...
        for ( ii = 2; ii <= field.number - 2; ii += 2 ) {
            ...
            for ( j = 2; j <= field.number - 1; j++ ) {
                ...
                uij = Complex ( field.real[ikij], field.imaginary[ikij] );
                ui1j =
                    Complex ( field.real[iki1j], field.imaginary[iki1j] );
                ui_1j =
                    Complex ( field.real[iki_1j],
                              field.imaginary[iki_1j] );
...
void elim (  )
{
    ...
    alpha[field.number].i = beta[field.number].r = beta[field.number].i =
        0.;
    ...
    cc = Csub ( c[i], Cmul ( a[i], alpha[i] ) );
    beta[field.number + 1] =
        Cdiv ( Cadd ( p[i], Cmul ( a[i], beta[i] ) ), cc );
    /*
     * edge amplitude =0 
     */
...
void all_mem (  )
{
    ...
    if ( ( a =
           ( fcomplex * ) calloc ( field.number + 3,
...
refraction coefficient equals to unity\n\n" );
    ...
    fprintf ( stderr,
              "steps 0.1 20 refr_d abs_d - performs 20 steps of 0.1 m in a medium\n\
absorbtion coefficient of which is given in a file abs_d,\n\
