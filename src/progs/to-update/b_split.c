...
int main(int argc, char *argv[]){
  ...
  fr=fopen(argv[1],"wb");
	...
	if(fwrite ((char *)&(field.fft_level), sizeof(int), 1, fr) != 1){
	    fprintf(stderr,"Error while writing FIELD.fft_level\n");
	    exit(1);
	}

	if(fwrite ((char *)&(field.sph_coords_factor), sizeof(double), 1, fr) != 1){
	    fprintf(stderr,"Error while writing FIELD.sph_coords_factor\n");
	    exit(1);
	}
